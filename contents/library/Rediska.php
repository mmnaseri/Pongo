...
class Rediska extends Rediska_Options {
	 ...
	 * @var array
	 */
	protected $_options = array('addToManager' => true, 'name' => self::DEFAULT_NAME, 'namespace' => '', 'servers' => array(array('host' => Rediska_Connection::DEFAULT_HOST, 'port' => Rediska_Connection::DEFAULT_PORT, 'weight' => Rediska_Connection::DEFAULT_WEIGHT,)), 'serializerAdapter' => 'phpSerialize', 'keyDistributor' => 'consistentHashing', 'redisVersion' => self::STABLE_REDIS_VERSION, 'profiler' => false,);
	...
	public function setServers(array $servers) {
		...
		foreach ($servers as $alias => $serverOptions) {
			...
			$this->addServer(isset($serverOptions['host']) ? $serverOptions['host'] : Rediska_Connection::DEFAULT_HOST, isset($serverOptions['port']) ? $serverOptions['port'] : Rediska_Connection::DEFAULT_PORT, $serverOptions);
		}
	...
	public function addServer($host, $port = Rediska_Connection::DEFAULT_PORT, array $options = array()) {
		...
		if ($this->_keyDistributor) {
			$this->_keyDistributor->addConnection($connectionString, isset($options['weight']) ? $options['weight'] : Rediska_Connection::DEFAULT_WEIGHT);
		}
	...
	public function subscribe($channelOrChannels, $timeout = null) {
		return new Rediska_PubSub_Channel($channelOrChannels, array('rediska' => $this, 'timeout' => $timeout, 'serverAlias' => $this->_specifiedConnection->getConnection()));
	}
	...
	public function monitor($timeout = null) {
		return new Rediska_Monitor(array('rediska' => $this, 'timeout' => $timeout, 'serverAlias' => $this->_specifiedConnection->getConnection()));
	}
	...
	public function getProfiler() {
		if (!$this->_profiler) {
			if (is_string($this->_options['profiler'])) {
				$this->_options['profiler'] = array('name' => $this->_options['profiler']);
			}
	 ...
	 * @return array
	 */
	public function getList($key, $start = 0, $end = -1, $responseIterator = false) {
		$args = func_get_args();
	 ...
	 * @return mixed
	 */
	public function popFromListBlocking($keyOrKeys, $timeout = 0, $pushToKey = null) {
		$args = func_get_args();
	 ...
	 * @return array
	 */
	public function getSortedSet($key, $withScores = false, $start = 0, $end = -1, $revert = false, $responseIterator = false) {
		$args = func_get_args();
	 ...
	 * @return array
	 */
	public function getFromSortedSetByScore($key, $min, $max, $withScores = false, $limit = null, $offset = null) {
		$args = func_get_args();
	 ...
	 * @return mixed
	 */
	public function unionSortedSets(array $keys, $storeKey, $aggregation = Rediska_Command_UnionSortedSets::SUM) {
		$args = func_get_args();
	 ...
	 * @return mixed
	 */
	public function intersectSortedSets(array $keys, $storeKey, $aggregation = Rediska_Command_IntersectSortedSets::SUM) {
		$args = func_get_args();
