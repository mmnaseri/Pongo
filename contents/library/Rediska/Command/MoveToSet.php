...
class Rediska_Command_MoveToSet extends Rediska_Command_Abstract {
	...
	public function create($fromKey, $toKey, $member) {
		...
		if ($fromKeyConnection === $toKeyConnection) {
			$command = array('SMOVE', $this->_rediska->getOption('namespace') . $fromKey, $this->_rediska->getOption('namespace') . $toKey, $member);
		} else {
			$this->setAtomic(false);
			$command = array('SISMEMBER', $this->_rediska->getOption('namespace') . $fromKey, $member);
		}
