...
class Rediska_Command_Rename extends Rediska_Command_Abstract {
	...
	public function create($oldKey, $newKey, $overwrite = true) {
		...
		if ($oldKeyConnection === $newKeyConnection) {
			...
			$command = array($overwrite ? 'RENAME' : 'RENAMENX', $this->_rediska->getOption('namespace') . $oldKey, $this->_rediska->getOption('namespace') . $newKey);
		} else {
			$this->setAtomic(false);

			$command = array('GET', $this->_rediska->getOption('namespace') . $oldKey);
		}
