	...
	public function create($keyOrKeys, $timeout = 0, $pushToKey = null) {
		...
		if ($pushToKey !== null) {
			...
			if ($connection != $this->_storeConnection) {
				...
				$command = array('BRPOP', $this->getRediska()->getOption('namespace') . $key);
			} else {
				$this->_throwExceptionIfNotSupported('2.1.7');

				$command = array('BRPOPLPUSH', $this->getRediska()->getOption('namespace') . $key, $this->getRediska()->getOption('namespace') . $pushToKey, $timeout);
			}
