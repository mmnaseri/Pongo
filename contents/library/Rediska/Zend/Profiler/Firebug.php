...
class Rediska_Zend_Profiler_Firebug extends Rediska_Profiler {
	...
	public function stopCallback(Rediska_Profiler_Profile $profile) {
		...
		$placeHolders = array('%rediskaName%' => $profile->getContext()->getRediska()->getName(), '%count%' => $this->count(), '%totalElapsedTime%' => $this->getTotalElapsedTime(4),);

		$label = str_replace(array_keys($placeHolders), array_values($placeHolders), $this->getLabel());
